      ...
      <div class="container">
        <div class="banner-hero banner-company">
          <div class="block-banner text-center">
            <h3 class="wow animate__animated animate__fadeInUp">{{__('Recherchez par entreprise')}}</h3>
            <div class="font-sm color-text-paragraph-2 mt-10 wow animate__animated animate__fadeInUp"
              data-wow-delay=".1s">{{__('Retrouvez toutes les entreprises incrites sur ').config('app.name')}}<br
                class="d-none d-xl-block">{{__('vous trouverez certainement des offres d\'emplois. qui sauront vous
              plaire')}}</div>
            <div class="box-list-character">
              <ul>
                @foreach(range('A', 'Z') as $letter)
                  <li><a href="#" wire:click='setActiveLetter("{{ $letter }}")' class="@if ($activeLetter === $letter) active @endif">{{ $letter }}</a></li>
                @endforeach
              </ul>
            </div>
          </div>
            ...
            <div class="content-page">
              <div class="box-filters-job">
                <div class="row">
                  ...
                  <div class="col-xl-6 col-lg-5"><span class="text-small text-showing">{{_('Affichage de')}} <strong>{{ $companies->firstItem() ? $companies->firstItem() : 0 }}-{{ $companies->lastItem() ? $companies->lastItem() : 0 }}</strong> {{__('de')}}
                      <strong>{{ $companies->total() }}</strong> jobs</span></div>
                  <div class="col-xl-6 col-lg-7 text-lg-end mt-sm-15">
                    <div class="display-flex2">
                      ...
                      <div class="box-view-type"><a class="view-type" href="jobs-list.html"><img
                            src="assets/imgs/template/icons/icon-list.svg" alt="jobBox"></a><a class="view-type"
                          href="jobs-grid.html"><img src="assets/imgs/template/icons/icon-grid-hover.svg"
                            alt="jobBox"></a></div>
                    </div>
                  </div>
                </div>
              ...
              <div class="row">
                ...
                <div class="col-xl-4 col-lg-4 col-md-6 col-sm-12 col-12">
                    <div class="card-grid-1 hover-up wow animate__animated animate__fadeIn">
                        <div class="image-box"><a href="{{ route('company.details', ['slug' => $company->slug])}}"><img
                                src="{{ asset('assets/imgs/brands/brand-1.png') }}" alt="{{$company->name}}"></a></div>
                        <div class="info-text mt-10">
                            <h5 class="font-bold"><a
                                    href="{{ route('company.details', ['slug' => $company->slug])}}">{{$company->name}}</a></h5>
                            <div class="mt-5"><img alt="{{$company->title}}" src="assets/imgs/template/icons/star.svg"><img alt="{{$company->title}}"
                                    {{-- src="assets/imgs/template/icons/star.svg"><img alt="{{$company->title}}"
                                    src="assets/imgs/template/icons/star.svg"><img alt="{{$company->title}}"
                                    src="assets/imgs/template/icons/star.svg"><img alt="{{$company->title}}"
                                    src="assets/imgs/template/icons/star.svg"><span --}}
                                    class="font-xs color-text-mutted ml-10">
                            </div><span class="card-location">{{$company->location->address}}</span>
                            <div class="mt-30"><a class="btn btn-grey-big"
                                    href="{{route('company.details', ['slug' => $company->slug])}}"><span>{{$company->jobs->count()}}</span><span>
                                        {{__('postes ouverts')}}</span></a></div>
                        </div>
                    </div>
                </div>
                @empty
                <div class="col-12">
                    <p>{{__('Aucun résultat')}}</p>
                </div>
                @endforelse
            </div>
            </div>
            ...
            <div class="sidebar-shadow none-shadow mb-30">
              <div class="sidebar-filters">
                <div class="filter-block head-border mb-30">
                  <h5>{{__('Filtre avancé')}}<a class="link-reset" wire:click="clearFilter" href="#">{{__('Effacer')}}</a></h5>
                </div>
                    ...
                    <ul class="list-checkbox">
                      @foreach($industries as $key => $industry)
                          <li>
                            ...
                            </label><span class="number-item">{{$industry->companies->count()}}</span>
                          </li>
                      @endforeach
                    </ul>
                  </div>
                  ...
                  <div class="card-grid-3 hover-up wow animate__animated animate__fadeIn">
                    ...
                    <div class="card-block-info">
                      ...
                      <p class="mt-10 color-text-paragraph font-sm">Our mission is to create the world&amp;rsquo;s most
                        sustainable healthcare company by creating high-quality healthcare products in iconic,
                    ...
                    <div class="card-block-info">
                      ...
                      <div class="card-2-bottom mt-20">
                        <div class="row">
                          ...
                          <div class="col-lg-6 text-end col-6 pt-15"><span class="color-text-paragraph-2 font-xs">6 mins
                              to read</span></div>
                        </div>
                      </div>
                    </div>
                ...
                <div class="swiper-slide">
                  <div class="card-grid-3 hover-up wow animate__animated animate__fadeIn">
                    ...
                    <div class="card-block-info">
                      ...
                      <p class="mt-10 color-text-paragraph font-sm">Learn how to respond if an interviewer asks you why
                        you dont have a degree, and read example answers that can help you craft</p>
                      <div class="card-2-bottom mt-20">
                        <div class="row">
                          <div class="col-lg-6 col-6">
                          ...
                          <div class="col-lg-6 text-end col-6 pt-15"><span class="color-text-paragraph-2 font-xs">9 mins
                              to read</span></div>
                        </div>
                      </div>
                    </div>
